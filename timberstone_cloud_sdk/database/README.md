# Timberstone Cloud SDK - mySQL Database

A simple example demonstrating how to write mySQL database using Pulumi.
...
1. Create a new stack:
    ...
    ```bash
    $ pulumi stack init aws-py-mysql
    ```
...
1. Set the AWS region and the names and passwords for admin and user:
    ...
    ```
    In the future, aws region will be captured via CI/CD pipeline.

1. Run `pulumi up -y` to deploy changes:

    ```bash
    Updating (aws-py-mysql):
        Type                                  Name                                           Status      
    +   pulumi:pulumi:Stack                   aws-py-mysql-aws-py-mysql  created     
